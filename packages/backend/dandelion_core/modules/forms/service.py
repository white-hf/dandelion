...
class FormService:
    ...
    def submit_form(self, submission: GenericSubmission):
        ...
        for field_cfg in fields:
            name = field_cfg['name']
            # Check both top-level and custom data fields
            val = submission.data.get(name) if name not in ["contact_name", "email", "phone", "business_name", "industry", "website_url", "city"] else getattr(submission, name, None)
